...
static size_t get_kernel_size(const struct fit_image_node *node)
{
	...
	printk(BIOS_WARNING, "Kernel size is %d\n", node->size);
...
static bool fit_place_mem(const struct range_entry *r, void *arg)
{
	...
	{
		printk(BIOS_DEBUG, "region is not ram %llu %lu\n", range_entry_size(r), range_entry_tag(r));
		return true;
	}
		

	printk(BIOS_DEBUG, "Offset %d, size %d\n", region->offset, region->size);
		      ...
		      struct region *initrd)
{
	...
	if (0 && !decompress_kernel_header(config->kernel)) {
	...
	printk(BIOS_DEBUG, "Found offset for kernel %d\n", kernel->size);
